...
def test_get_kontakt_not_found(lambda_context, kontakt_table):
    pathParameters = {"id": "unknown_id"}
    response = kontakt_handler.handle(
        event("/api/kontakt/{id}", "GET", None, pathParameters), lambda_context
    )
...
def test_get_kontakt_ok(lambda_context, kontakt_table):
    ...
    createdKontakt = kontakt_controller.create_kontakt(DEFAULT_TENANT_ID, item, True)

    pathParameters = {"id": createdKontakt.id}
    response = kontakt_handler.handle(
        event("/api/kontakt/{id}", "GET", None, pathParameters), lambda_context
    )
...
def test_get_kontakt_without_tenant_id_not_ok(lambda_context, kontakt_table):
    ...
    pathParameters = {"id": createdKontakt.id}
    response = kontakt_handler.handle(
        event("/api/kontakt/{id}", "GET", None, pathParameters, headers), lambda_context
    )

    assert response == lambda_response(
        400, json.dumps({"error_text": "tenant not present."})
    )
